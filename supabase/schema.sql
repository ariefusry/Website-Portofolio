...
-- ------------------------------------------------------------- kunjungan --
-- Penghitung kunjungan yang tampil di footer. Satu baris, dinaikkan lewat
-- fungsi di bawah — bukan lewat `update` dari klien.
create table if not exists public.site_metrics (
  id smallint primary key default 1 check (id = 1),
  visits bigint not null default 0
);

insert into public.site_metrics (id, visits)
values (1, 0)
on conflict (id) do nothing;

alter table public.site_metrics enable row level security;

-- Boleh dibaca siapa saja; tidak ada policy tulis, jadi klien tidak bisa
-- mengarang angkanya lewat REST.
drop policy if exists site_metrics_read on public.site_metrics;
create policy site_metrics_read on public.site_metrics
  for select to anon, authenticated using (true);

/*
 * Satu-satunya jalan menaikkan penghitung. `security definer` membuatnya
 * melewati RLS, sehingga tabelnya bisa tetap read-only dari luar.
 *
 * Ini tidak mencegah seseorang memanggil fungsinya berulang kali — penghitung
 * kunjungan di sisi klien memang tidak bisa. Yang dijaga di sini adalah
 * angkanya tidak bisa di-set sembarang nilai, hanya bertambah satu per panggilan.
 */
