...
values
  ('indeta', 'indeta', 'INDETA',
   ...
   'Platform pariwisata: destinasi, produk UMKM, paket wisata, dan artikel, dengan ulasan pengunjung dan CMS admin. Laravel merender, Supabase mengerjakan sisanya.',
   array['PAID CLIENT','SOLO FULLSTACK'], true, array['Laravel','Supabase','PostgreSQL','Tailwind','Vite','Vercel'],
   false, true,
   ...
   'Pengunjung menelusuri destinasi per kategori, membaca artikel, melihat produk UMKM lokal dan paket wisata, lalu meninggalkan rating dan ulasan; dashboard admin menangani CRUD penuh atas semuanya, lengkap dengan statistik langsung dan unggah gambar. Arsitekturnya yang layak dijelaskan: Laravel 12 tetap jadi lapisan tipis untuk routing dan render Blade, sementara pengambilan data, auth, CRUD, dan unggah berkas seluruhnya berjalan di sisi klien lewat Supabase JS SDK. Jadi framework-nya merender dan BaaS yang memegang data — tanpa lapisan API buatan saya di antaranya. Bisa dipasang sebagai PWA, di-deploy serverless di Vercel lewat vercel-php.',
   '[{"label":"ROLE","value_en":"Sole fullstack developer","value_id":"Fullstack developer tunggal"},
     {"label":"ARCHITECTURE","value_en":"Laravel renders, Supabase JS SDK handles data","value_id":"Laravel merender, Supabase JS SDK menangani data"},
     {"label":"SCOPE","value_en":"Public site + admin CMS + PWA","value_id":"Situs publik + CMS admin + PWA"},
     {"label":"DEPLOY","value_en":"Serverless on Vercel via vercel-php","value_id":"Serverless di Vercel lewat vercel-php"}]'::jsonb,
   array[
  ...
  ('sugih', 'sugih', 'SUGIH',
   ...
   'Company profile merek kretek di bawah CV. Prioritas Group, dengan CMS buatan sendiri di belakangnya. Dikerjakan bersama satu developer lain.',
   array['PAID CLIENT · 2 DEVS'], true, array['Laravel','Tailwind','Alpine.js','GSAP','PostgreSQL','Vercel'], true, true,
   'A public site — landing page, company history, product catalogue, news, careers and contact — sitting on an admin panel written from scratch rather than pulled from a package. The client edits products, articles and job openings there, reorders products by dragging them, and attaches sub-items to each product. Being a tobacco brand, every page opens behind the 21+ excise warning Indonesian law expects. Laravel 11 with Blade and Tailwind on the surface; Alpine, Swiper, GSAP with Lenis and Atropos handle the motion. Postgres on Supabase, deployed serverless on Vercel.',
   'Situs publik — landing page, sejarah perusahaan, katalog produk, berita, karir, dan kontak — berdiri di atas panel admin yang ditulis sendiri, bukan diambil dari package. Klien menyunting produk, artikel, dan lowongan di sana, mengurutkan produk dengan menyeretnya, dan menautkan sub-item ke tiap produk. Karena ini merek tembakau, setiap halaman dibuka di balik peringatan cukai 21+ sebagaimana diminta aturan di Indonesia. Laravel 11 dengan Blade dan Tailwind di permukaan; Alpine, Swiper, GSAP dengan Lenis, dan Atropos menangani geraknya. Postgres di Supabase, di-deploy serverless di Vercel.',
   '[{"label":"ROLE","value_en":"Fullstack, with one other developer","value_id":"Fullstack, bersama satu developer lain"},
     {"label":"SCOPE","value_en":"Public site + custom admin CMS","value_id":"Situs publik + CMS admin buatan sendiri"},
     {"label":"STACK","value_en":"Laravel 11 · Blade · Tailwind · Alpine","value_id":"Laravel 11 · Blade · Tailwind · Alpine"},
     {"label":"DEPLOY","value_en":"Supabase Postgres, serverless on Vercel","value_id":"Postgres Supabase, serverless di Vercel"}]'::jsonb,
   array[
